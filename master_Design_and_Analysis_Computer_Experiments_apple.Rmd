...
experiment_path <- path.expand("~/temp_experiment.xml")
...
# I wasn't able to test this on a MAC unfortunately
system.time(
  number_burned_60_par <- unlist(mclapply(cl, rep(60,100), runModel))
  )
...
number_burned <- unlist(mclapply(densities_rep, runModel))
...
number_burned <- unlist(mclapply(prior, runModel))
proportion_burned <- number_burned /(possible_trees * (prior/100.0 ))  

hist(proportion_burned)

number_burned_mean <- unlist(mclapply(rep(56,50), runModel))
