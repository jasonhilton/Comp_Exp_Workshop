...
Now lets run the simulation 50 times at the density 60 to start to examine what the output *distribution* looks like at this point. When we are doing this sort of experiment it is useful to check how long each set of run is going to take - otherwise we might be waiting for simulations that will never end.
The function'*system.time*' is our friend in this context.
...
system.time(proportion_burned_50<-c( proportion_burned_50, parSapply(cl, rep(60,50), runModel)))

```
Hopefully that should have been quicker than the first time! If not this is probably due to the overhead associated with setting up the clusters. Note that if you are running this on a Mac or a LINUX based system such as Ubuntu, then you might be able to use the much simpler mclapply function (which, in my experience, is also faster).

```{r}
hist(proportion_burned_50,breaks=15)

```

We now have 100 runs, and we can see from the above histogram that the distribution is pretty irregular. Sometimes, just the mean of the output will not give you adequate information about the whole of the distribution. 
Lets try and build up a picture of the uncertainty caused by simulation stochasticity across the rest of the parameter space. 
...
Our prior states that the desnity of the forest has a normal distribution with mean .56 and sd 0.05. Sample from this distribution using rnorm, and run the simulation at the sampled points. Plot the density of this output distribution. 
...
The dice kriging command fits a standard linear model - called the trend - but then also fits a gaussian process to the residuals. The output tells us that the a matern5_2 kernel was used - which define the shape of the covariance function. Other kernels are available - experiment with them if you wish.  It also gives us the estimates of the scaling parameters, and the variance and nugget effects. 

The nugget effect here accounts for simulation stochasticity, while the variance parameter refers to variance relating to the simulation inputs.
...
4. Now, click on the options tab. We have two inputs, so input '2' in the number of inputs box. You can name them appropriately if you wish
